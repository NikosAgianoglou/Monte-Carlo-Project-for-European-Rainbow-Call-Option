...
double MC_Call_Price_Rainbow(double S1,double S2,double K,double r,double v1,double v2,double T,double p,int num_sims)  
 {
	...
	double anti_payoff_sum=0.0;
	double anti_payoff_squared_sum=0.0;  
 	...
 	for (int i=0; i<num_sims; i++){
		...
		double S_T1_a=S1*exp(nu_T1+v_T1*(-epsilon1));
		double S_T2_a=S2*exp(nu_T2+v_T2*(-epsilon2));
		double anti_payoff=std::max(std::min(S_T1_a,S_T2_a)-K,0.0);
		double anti_payoff_squared=anti_payoff*anti_payoff;
		double anti_mean=(disc_payoff+anti_payoff)*0.5;
		anti_payoff_sum+=anti_mean;
		anti_payoff_squared_sum+=(anti_mean*anti_mean);
	}

double anti_payoff_average=anti_payoff_sum/num_sims;
double price_anti=(anti_payoff_average)*std::exp(-r*T);
anti_sample_var=((anti_payoff_squared_sum-num_sims*anti_payoff_average*anti_payoff_average)/(num_sims-1))*std::exp(-2.0*r*T);
...
int main() {
    ...
    double rel_error = z * std::sqrt(anti_sample_var / num_sims) / call;
    
    cout << "--- ANTITHETIC VARIATES METHOD ---" << endl;
