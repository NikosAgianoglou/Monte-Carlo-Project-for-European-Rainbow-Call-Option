...
double control_sample_var=0.0;

// Pricing a European Call Option with the Monte Carlo method
double MC_Call_Price_Rainbow(double S1,double S2,double K,double r,double v1,double v2,double T,double p,int num_sims, int num_pilot)
{
	...
	double S_T1_sum=0.0;
	double S_T2_sum=0.0;
	double control_payoff_sum_pilot=0.0;
	double product_sum1=0.0;
	double product_sum2=0.0;
	
	for (int i=0;i<num_pilot;i++){
		double epsilon1=Standard_Normal_Rand();
		double epsilon3=Standard_Normal_Rand();
		double epsilon2=p*epsilon1+sqrt(1-p*p)*(epsilon3);
	
		//Monte Carlo with Control Variates
		double S_T1=S1*exp(nu_T1+v_T1*epsilon1);
		double S_T2=S2*exp(nu_T2+v_T2*epsilon2);
		double control_payoff=std::max(std::min(S_T1,S_T2)-K,0.0);
		S_T1_sum+=S_T1;
		S_T2_sum+=S_T2;
   		control_payoff_sum_pilot+=control_payoff;
    	product_sum1+=S_T1*control_payoff;
    	product_sum2+=S_T2*control_payoff; 
	}
	
double sample1_cov=(product_sum1-(S_T1_sum*control_payoff_sum_pilot)/num_pilot)/(num_pilot - 1);
double sample2_cov=(product_sum2-(S_T2_sum*control_payoff_sum_pilot)/num_pilot)/(num_pilot - 1);

double Var1=S1*S1*exp(2.0*r*T)*(exp(v1*v1*T)-1.0); 
double Var2=S2*S2*exp(2.0*r*T)*(exp(v2*v2*T)-1.0);
double Y1Y2_cov=S1*S2*exp(2.0*r*T)*(exp(p*v1*v2*T)-1.0);

double det=Var1*Var2-Y1Y2_cov*Y1Y2_cov;
double c1=(Y1Y2_cov*sample2_cov-sample1_cov*Var2)/det;;
double c2=(Y1Y2_cov*sample1_cov-sample2_cov*Var1)/det;;

double ExpY1=S1*exp(r*T);
double ExpY2=S2*exp(r*T);	
...
double control_var_squared_sum = 0.0;
	...
	for (int i=0; i<num_sims; i++){
		...
		double controlled_payoff=disc_payoff+c1*(S_T1-ExpY1)+c2*(S_T2-ExpY2);
		double controlled_payoff_squared=controlled_payoff*controlled_payoff;
		control_var_sum+=controlled_payoff;
		control_var_squared_sum+=controlled_payoff_squared;
	}
    
double control_payoff_average=control_var_sum/num_sims;
double price_control=control_payoff_average*std::exp(-r*T);
control_sample_var=((control_var_squared_sum-num_sims*control_payoff_average*control_payoff_average)/(num_sims-1))*std::exp(-2.0*r*T);
...
int main() {
    ...
    int num_pilot = 200000;   // Number of pilot simulated asset paths
    ...
    double call = MC_Call_Price_Rainbow(S1, S2, K, r, v1, v2, T, p, num_sims,num_pilot);

    // Finally we output the parameters and prices
    
    // Relative error for alpha = 5% (95% confidence)
    double z = inverse_of_normal_cdf(0.975, 0.0, 1.0);
    double rel_error = z * std::sqrt(control_sample_var/num_sims)/call;
    
    cout << "--- CONTROL VARIATES METHOD ---" << endl;
    cout << "Number of Paths: " << num_sims << std::endl;
    cout << "Number of Pilot Paths: " << num_pilot << std::endl;
